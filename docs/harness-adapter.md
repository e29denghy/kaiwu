...
KAIWU ships a process-separated bridge under [`bridges/deepseek-harness`](../bridges/deepseek-harness). It pins the latest installable npm headless CLI `0.1.1-rc.2`, records Python SDK compatibility against `0.1.1rc1`, translates each approved Quest into one DSH task, and translates the selected automation transport's lifecycle/results back into the event contract above. Auto mode uses the npm CLI on every platform because the `0.1.1rc1` macOS SDK runtime cannot load the sandbox plugins required by KAIWU's external Cordis composition. The later GitHub-only `0.1.2-alpha.1` tag has no matching npm or PyPI package and is not selected.

The bridge maps `requires_write=false` to DSH `read-only` and write-capable Quests to `workspace-write`. Auto/CLI mode's shipped headless profile has no interactive approval answerer. Explicit SDK mode requests sandboxed Bash and filesystem providers with approval policy `never`; missing runtime plugins abort initialization. Both paths fail closed on unattended permission escalation. KAIWU imports the resulting events separately and projects recognized `execution.started`, `execution.completed`, `execution.failed`, and `execution.cancelled` events onto the matching `QuestExecution` by `payload.dispatch_id`.
